...
/* Compute magnitude^2 for a Goertzel bin (same approach as DTMF decoder). */
...
static void decode_pattern(plcode_cwid_dec_t *c)
{
    char ch;
...
static void process_block(plcode_cwid_dec_t *c)
{
    ...
    if (current_tone && !c->tone_on) {
        /* Tone onset — check if preceding gap was a character or word boundary */
        if (c->pattern_len > 0) {
            if (c->gap_samples >= 5 * c->dot_samples) {
                /* Word gap: decode char, then insert space */
                decode_pattern(c);
                if (c->message_len < PLCODE_CWID_MSG_MAX - 1) {
                ...
                /* Character gap: decode accumulated pattern */
                decode_pattern(c);
            }
        }
        c->tone_samples = 0;
    } else if (!current_tone && c->tone_on) {
        /* Tone offset — classify duration as dot or dash */
    ...
    if (current_tone) {
        ...
        /* Character gap timeout — decode if gap is long enough */
        if (c->gap_samples >= 2 * c->dot_samples && c->pattern_len > 0) {
            if (c->gap_samples >= 5 * c->dot_samples &&
                c->message_len > 0 &&
                c->message[c->message_len - 1] != ' ') {
                /* Crossed word-gap threshold: decode then add space */
                decode_pattern(c);
                if (c->message_len < PLCODE_CWID_MSG_MAX - 1) {
                    c->message[c->message_len++] = ' ';
                    c->message[c->message_len] = '\0';
                ...
                /* Just crossed char-gap threshold this block */
                decode_pattern(c);
            }
        }
    }
...
int plcode_cwid_dec_create(plcode_cwid_dec_t **ctx, int rate, int freq, int wpm)
{
    plcode_cwid_dec_t *c;
    ...
    c = (plcode_cwid_dec_t *)calloc(1, sizeof(*c));
