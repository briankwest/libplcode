#include "plcode_internal.h"
#include <stdlib.h>

int plcode_toneburst_enc_create(plcode_toneburst_enc_t **ctx,
                                 int rate, int freq, int duration_ms,
                                 int16_t amplitude)
{
    plcode_toneburst_enc_t *c;

    if (!ctx) return PLCODE_ERR_PARAM;
    if (!plcode_valid_rate(rate)) return PLCODE_ERR_RATE;
    if (freq < 300 || freq > 3000) return PLCODE_ERR_PARAM;
    if (duration_ms <= 0) return PLCODE_ERR_PARAM;
    if (amplitude <= 0) return PLCODE_ERR_PARAM;

    plcode_tables_init();

    c = (plcode_toneburst_enc_t *)calloc(1, sizeof(*c));
    if (!c) return PLCODE_ERR_ALLOC;

    c->phase_inc = (uint32_t)((double)freq / (double)rate * 4294967296.0 + 0.5);
    c->amplitude = amplitude;
    c->total_samples = (int)((double)rate * duration_ms / 1000.0 + 0.5);
    c->phase = 0;
    c->cur_sample = 0;
    c->complete = 0;

    *ctx = c;
    return PLCODE_OK;
}

void plcode_toneburst_enc_process(plcode_toneburst_enc_t *ctx,
                                   int16_t *buf, size_t n)
{
    size_t i;
    if (!ctx || !buf) return;

    for (i = 0; i < n && !ctx->complete; i++) {
        int16_t tone = plcode_scale(plcode_sine_lookup(ctx->phase),
                                     ctx->amplitude);
        buf[i] = plcode_clamp16((int32_t)buf[i] + (int32_t)tone);
        ctx->phase += ctx->phase_inc;
        ctx->cur_sample++;
        if (ctx->cur_sample >= ctx->total_samples)
            ctx->complete = 1;
    }
}

int plcode_toneburst_enc_complete(plcode_toneburst_enc_t *ctx)
{
    if (!ctx) return 1;
    return ctx->complete;
}

void plcode_toneburst_enc_destroy(plcode_toneburst_enc_t *ctx)
{
    free(ctx);
}
